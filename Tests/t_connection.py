...
class Test00_Connection(SlapdTestCase):
    """
    test LDAP search operations
    """

    ldap_object_class = Connection
    ...
    def test_search_keys_are_text(self):
        base = self.server.suffix
        l = self._ldap_conn
        responses = l.search_s(base, ldap.SCOPE_SUBTREE, '(cn=Foo*)', ['*'])
        entries, result = responses[:-1], responses[-1]
        entries = sorted((e.dn, e.attrs) for e in entries)
        dn, fields = entries[0]
    ...
    def test_filterstr_accepts_unicode(self):
        l = self._ldap_conn
        base = self.server.suffix
        responses = l.search_s(base, ldap.SCOPE_SUBTREE, '(cn=abc\U0001f498def)', ['*'])
        entries, result = responses[:-1], responses[-1]
        self.assertEqual(entries, [])
        self.assertIsInstance(result, ldap.response.SearchResult)

    def test_attrlist_accepts_unicode(self):
        base = self.server.suffix
        responses = self._ldap_conn.search_s(
            base, ldap.SCOPE_SUBTREE,
            '(cn=Foo*)', ['abc', 'abc\U0001f498def'])
        entries, result = responses[:-1], responses[-1]

        for entry in entries:
            self.assertIsInstance(entry.dn, str)
            self.assertEqual(entry.attrs, {})
    ...
    def test_find_unique_entry(self):
        entry = self._ldap_conn.find_unique_entry(
            self.server.suffix,
            ldap.SCOPE_SUBTREE,
            '(cn=Foo4)',
            ['cn'],
    ...
    def test004_enotconn(self):
        ...
        try:
            m = l.simple_bind_s("", "")
            r = l.result(m, ldap.MSG_ALL, self.timeout)
    ...
    def test_result_compare_true(self):
        base = self.server.suffix
        l = self._ldap_conn
        msgid = l.compare('cn=Foo1,%s' % base, 'cn', b'Foo1')
        responses = l.result()
        result, = responses
        self.assertEqual(result.msgid, msgid)
        self.assertIsInstance(result, ldap.response.CompareResult)
        result.raise_for_result()
        self.assertEqual(result.result, ldap.COMPARE_TRUE.errnum)
        self.assertTrue(result)

    def test_result_compare_false(self):
        base = self.server.suffix
        l = self._ldap_conn
        msgid = l.compare('cn=Foo1,%s' % base, 'cn', b'Foo2')
        responses = l.result()
        result, = responses
        self.assertEqual(result.msgid, msgid)
        self.assertIsInstance(result, ldap.response.CompareResult)
        result.raise_for_result()
        self.assertEqual(result.result, ldap.COMPARE_FALSE.errnum)
        self.assertFalse(result)

    def test_result_compare_notfound(self):
        base = self.server.suffix
        l = self._ldap_conn
        msgid = l.compare('cn=invalid,%s' % base, 'cn', b'Foo2')
        responses = l.result()
        result, = responses
        self.assertEqual(result.msgid, msgid)
        self.assertIsInstance(result, ldap.response.CompareResult)
        self.assertEqual(result.result, ldap.NO_SUCH_OBJECT.errnum)
        with self.assertRaises(ldap.NO_SUCH_OBJECT):
            result.raise_for_result()
    ...
    def test_result_compare_invalidattr(self):
        base = self.server.suffix
        l = self._ldap_conn
        msgid = l.compare('cn=Foo1,%s' % base, 'invalidattr', b'invalid')
        responses = l.result()
        result, = responses

        self.assertEqual(result.msgid, msgid)
        self.assertIsInstance(result, ldap.response.CompareResult)
        self.assertEqual(result.result, ldap.UNDEFINED_TYPE.errnum)
        with self.assertRaises(ldap.UNDEFINED_TYPE):
            result.raise_for_result()
    ...
    def test_passwd_s(self):
        ...
        self.assertIsInstance(result, ldap.response.AddResult)
        self.assertEqual(result.msgtype, ldap.RES_ADD)
        self.assertIsInstance(result.msgid, int)
        self.assertEqual(result.controls, None)

        # try changing password with a wrong old-pw
        with self.assertRaises(ldap.UNWILLING_TO_PERFORM):
            l.passwd_s(dn, "bogus", "ignored").raise_for_result()

        # have the server generate a new random pw
        res = l.passwd_s(dn, "initial", newpw=None).raise_for_result()
        self.assertEqual(res.name, None)

        password = res.genPasswd
        self.assertIsInstance(password, bytes)

        # try changing password back
        res = l.passwd_s(dn, password, "initial").raise_for_result()
        self.assertEqual(res.name, None)
    ...
    def test_slapadd(self):
        with self.assertRaises(ldap.INVALID_DN_SYNTAX):
            self._ldap_conn.add_s(
                "myAttribute=foobar,ou=Container,%s" % self.server.suffix, [
...
@requires_init_fd()
class Test01_ConnectionWithFileno(Test00_Connection):
