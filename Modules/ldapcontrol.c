...
static void LDAPControl_DEL(LDAPControl *lc) {
	if (lc == NULL)
		return;
...
void LDAPControl_List_DEL(LDAPControl **lcs) {
	LDAPControl **lcp;
...
static LDAPControl *Tuple_to_LDAPControl(PyObject *tup) {
	char *oid;
	char iscritical;
...
int LDAPControls_from_object(PyObject *list, LDAPControl ***controls_ret) {
	Py_ssize_t len, i;
	LDAPControl **ldcs;
	LDAPControl *ldc;
	PyObject *item;
...
PyObject *LDAPControls_to_List(LDAPControl **ldcs) {
	...
	for (i = 0; i < num_ctrls; i++) {
		pyctrl = Py_BuildValue("sbO&", ldcs[i]->ldctl_oid, ldcs[i]->ldctl_iscritical, LDAPberval_to_object, &ldcs[i]->ldctl_value);
		if (pyctrl == NULL) {
			Py_DECREF(res);
			return NULL;
		}
...
PyObject *LDAPMod_encode_rfc3876(PyObject *module, PyObject *args) {
	PyObject *res = 0;
	int err;
	BerElement *vrber = 0;
	char *vrFilter;
...
PyObject *LDAPMod_encode_rfc2696(PyObject *module, PyObject *args) {
	PyObject *res = 0;
	BerElement *ber = 0;
...
PyObject *LDAPMod_decode_rfc2696(PyObject *module, PyObject *args) {
	PyObject *res = 0;
	BerElement *ber = 0;
...
PyObject *LDAPMod_encode_assertion_control(PyObject *module, PyObject *args) {
	int err;
	PyObject *res = 0;
