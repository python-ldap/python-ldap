        ...
        if ( res_type == LDAP_RES_SEARCH_ENTRY ) {
            ...
            if ( res_type == LDAP_RES_EXTENDED ) {
                ...
                if ( (rc = ldap_parse_sasl_bind_result( self->ldap, msg,
                            &credentials, 0 )) != LDAP_SUCCESS ) {
                    goto error;
                }

                if ( credentials ) {
                    pytmp = LDAPberval_to_object( credentials );
                    ber_bvfree( credentials );
                    if ( !pytmp || PyDict_SetItemString( data, "credentials", pytmp ) ) {
                        goto error;
                    }
                    Py_DECREF( pytmp );
                } else {
                    pytmp = NULL;
                    if ( PyDict_SetItemString( data, "credentials", Py_None ) ) {
                        goto error;
                    }
                }
            }
