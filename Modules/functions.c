...
PyObject *LDAPMod_initialize(PyObject *module, PyObject *args) {
	char *uri;
	LDAP *ld = NULL;
	int ret;
	PyThreadState *save;
...
PyObject *LDAPMod_initialize_fd(PyObject *module, PyObject *args) {
	char *url;
	LDAP *ld = NULL;
	int ret;
	int fd;
...
PyObject *LDAPMod_str2dn(PyObject *module, PyObject *args) {
	struct berval str;
	LDAPDN dn;
	int flags = 0;
	...
	for (i = 0; dn && dn[i]; i++) {
		...
		for (j = 0; rdn[j]; j++) {
			...
			tuple = Py_BuildValue("(O&O&i)", LDAPberval_to_unicode_object, &ava->la_attr, LDAPberval_to_unicode_object, &ava->la_value, ava->la_flags & ~(LDAP_AVA_FREE_ATTR | LDAP_AVA_FREE_VALUE));
			if (!tuple) {
				Py_DECREF(rdnlist);
				goto failed;
			}
...
static void _free_dn_structure(LDAPDN dn) {
	if (dn == NULL)
		return;
 ...
 */
PyObject *LDAPMod_dn2str(PyObject *self, PyObject *args) {
...
PyObject *LDAPMod_set_option(PyObject *module, PyObject *args) {
	PyObject *value;
	int option;
...
PyObject *LDAPMod_get_option(PyObject *module, PyObject *args) {
	int option;
...
PyObject *LDAPMod_is_filter(PyObject *module, PyObject *args) {
	const char *filter;
	BerElement *ber;
	int rc;
