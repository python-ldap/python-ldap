 ...
 */
static PyObject *option_error(int res, const char *fn) {
		...
		if (value == Py_None) {
			...
			if (!PyArg_Parse(value, "d:set_option", &doubleval)) {
				if (PyErr_ExceptionMatches(PyExc_TypeError)) {
					...
					PyErr_Clear();
					PyErr_Format(PyExc_TypeError, "A float or None is expected for timeout, got %S", Py_TYPE(value));
				}
				return 0;
			}
		}
...
PyObject *LDAP_get_option(LDAPObject *self, int option) {
	...
	case LDAP_OPT_API_INFO:
		...
		v = Py_BuildValue("{s:i, s:i, s:i, s:s, s:i, s:O}", "info_version", apiinfo.ldapai_info_version, "api_version", apiinfo.ldapai_api_version, "protocol_version", apiinfo.ldapai_protocol_version,
		                  "vendor_name", apiinfo.ldapai_vendor_name, "vendor_version", apiinfo.ldapai_vendor_version, "extensions", extensions);
