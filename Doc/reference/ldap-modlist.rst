...
.. function:: modifyModlist( old_entry, new_entry [, ignore_attr_types=[] [, ignore_oldexistent=0 [, case_ignore_attr_types=None]]]) -> list
   ...
   *ignore_attr_types* is a list of attribute type
   ...
   .. note::
      Replacing attribute values would normally be done with
      :py:const:`ldap.MOD_REPLACE`. Unlike deleting specific values,
      :py:const:`ldap.MOD_REPLACE` does not require an EQUALITY matching rule
      to be defined for the attribute in the server's subschema.

      The modlist generated by this function uses a
      :py:const:`ldap.MOD_DELETE`/:py:const:`ldap.MOD_ADD` pair instead, as a
      work-around for potential issues with :py:const:`ldap.MOD_REPLACE` on
      some LDAP servers. This may fail if the server performs schema checks
      on the transient entry state while processing the modify operation.
