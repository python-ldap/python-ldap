...
class LDIFWriter:
    ...
    def _needs_base64_encoding(self, attr_type: str, attr_value: bytes) -> int:
        ...
        """
        return attr_type.lower() in self._base64_attrs or safe_string_re.search(attr_value) is not None
    ...
    def _unparseChangeRecord(self, modlist: LDAPModList) -> None:
        ...
        for mod in modlist:
            ...
            if mod_len == 3:
                ...
                self._unparseAttrTypeandValue(MOD_OP_STR[mod_op], mod_type.encode('ascii'))
            elif mod_len == 2:
...
def CreateLDIF(
    ...
    """
    warnings.warn(
        'ldif.CreateLDIF() is deprecated. Use LDIFWriter.unparse() instead. It will be removed in python-ldap 3.1',
...
class LDIFParser:
    ...
    def _file_sends_bytes(self) -> bool:
        return self._binary_input_file is not None

    @property
    def _input_file(self) -> BinaryIO | TextIO | None:
        return self._binary_input_file or self._text_input_file
    ...
    def handle_add(self, dn: str, entry: LDAPEntryDict, controls: LDAPControlTuples | None = None) -> None:
        """
    ...
    def parse_change_records(self) -> None:
        ...
        while k is not None and (not self._max_entries or self.records_read < self._max_entries):
            ...
            if changetype == 'modify':
                ...
                if k is not None:
                    raise ValueError(f'Line {self.line_counter}: Unexpected attribute {k} in LDIF delete')
                self.handle_delete(dn, controls or None)
            elif changetype in ('moddn', 'modrdn', 'rename'):
                if k is None or k.lower() != 'newrdn':
                    raise ValueError(f'Line {self.line_counter}: expected "newrdn"got {k}')
                if v is None:
                ...
                if k is None or k.lower() != 'deleteoldrdn':
                    raise ValueError(f'Line {self.line_counter}: expected "deleteoldrdn" got {k}')
                if v not in (b'0', b'1'):
                    raise ValueError(f'Line {self.line_counter}: invalid value for "deleteoldrdn": {k}')
                deleteoldrdn = v == b'1'
                try:
                    k, v = next_key_and_value()
                except EOFError:
                ...
                if k is not None:
                    if k.lower() != 'newsuperior':
                        raise ValueError(f'Line {self.line_counter}: expected "newsuperior" got {k}')
                    if v is None:
                        raise ValueError(f'Line {self.line_counter}: newrdn without a value')
                    ...
                    try:
                        ...
                        k, v = None, None
                if k is not None:
                    raise ValueError(f'Line {self.line_counter}: {changetype} entry unexpected pseudoattribute {k}')
                self.handle_modrdn(dn, newrdn, deleteoldrdn, newsuperior, controls or None)
            else:
...
class LDIFRecordList(LDIFParser):
    ...
    def handle_add(self, dn: str, entry: LDAPEntryDict, controls: LDAPControlTuples | None = None) -> None:
        ...
        This method should be implemented by applications using LDIFParser.
        """
        self.all_changes.append(('add', {'dn': dn, 'entry': entry, 'controls': controls}))
    ...
    def handle_modify(
        ...
        self.all_changes.append(('modify', {'dn': dn, 'modops': modops, 'controls': controls}))
...
def ParseLDIF(f: TextIO | BinaryIO, ignore_attrs: list[str] | None = None, maxentries: int = 0) -> list[tuple[str, LDAPEntryDict]]:
    ...
    if ignore_attrs is None:
        ignore_attrs = []
    warnings.warn(
        'ldif.ParseLDIF() is deprecated. Use LDIFRecordList.parse() instead. It will be removed in python-ldap 3.1',
        category=DeprecationWarning,
        stacklevel=2,
    )
    ldif_parser = LDIFRecordList(f, ignored_attr_types=ignore_attrs, max_entries=maxentries)
    ldif_parser.parse()
