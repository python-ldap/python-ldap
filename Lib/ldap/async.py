...
class WrongResultType(Exception):
    """
    raised in case wrong result type was received in results
    """
...
class AsyncSearchHandler:
    ...
    def startSearch(
        ...
        """
        self._msg_id = self._ldap_conn.search_ext(
            searchRoot,
            searchScope,
            filterStr,
            attrList,
        ...
        try:
            ...
            while go_ahead:
                while result_type is None and not result_list:
                    result_type, result_list, _, _ = self._ldap_conn.result3(
                        self._msg_id,
                        0,
                        timeout
                    )
            ...
            if partial and self._msg_id != None:
                self._ldap_conn.abandon(self._msg_id)
        self.postProcessing()
...
class IndexedDict(Dict):
    ...
    def _processSingleResult(self, resultType, resultItem):
        if resultType in ENTRY_RESULT_TYPES:
            ...
            dn, entry = resultItem
            self.allEntries[dn] = entry
            for attr_type in self.indexed_attrs:
                if attr_type in entry:
                    for attr_value in entry[attr_type]:
                        try:
                            self.index[attr_type][attr_value].append(dn)
                        except KeyError:
                            self.index[attr_type][attr_value] = [dn]
