...
from typing import Any, Dict, Iterable, List, MutableMapping, Set, Tuple, Type, TypeVar
...
for o in list(vars().values()):
  if issubclass(o, SchemaElement) and hasattr(o, 'schema_attribute'):
...
class SubSchema:
  ...
  def tree(
    self,
    schema_element_class: Type[ObjectClass] | Type[AttributeType],
    schema_element_filters: Iterable[Tuple[str, Iterable[str | int]]] | None = None,
  ) -> cidict[List[str]]:
    """
  ...
  def attribute_types(
    ...
    r_must: cidict[ldap.schema.AttributeType | None] = ldap.cidict.cidict()
    r_may: cidict[ldap.schema.AttributeType | None] = ldap.cidict.cidict()
...
def urlfetch(
    uri: str,
    trace_level: int = 0,
  ) -> Tuple[str | None, ldap.schema.subentry.SubSchema | None]:
  """
  ...
  subschemasubentry_entry: LDAPEntryDict = ldap.cidict.cidict()
  s_temp = s_temp or {}
